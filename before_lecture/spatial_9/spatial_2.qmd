...
here, data.area should return 0 because area of a point is 0 (but it doesn't idk why)
...
data.union_all().envelope works for me
...
when we ran envelope, it gave us back a polygon. here, we get back a multipolygon. bc not all the dense census tracts are next to each other. so only the dense/not dense that have boundaries next to each other are dissolved. 
...
### see section 6.7 in the textbook
